...
def plot_waveform(wavedata_ndarray:np.ndarray,sampling_rate:float,picture_name_option:str=""):
    # 波形を表示する準備
...
def get_spectrogram(wavedata_ndarray:np.ndarray,n_samples:int,skip_width:int):
    # 空の行列を準備
    spectrogram=np.zeros((1+(len(wavedata_ndarray)-n_samples)//skip_width,n_samples//2),dtype=np.complex128)
...
def plot_spectrogram(spectrogram:np.ndarray,sampling_rate:float,n_samples:int,skip_width:int):
    # スペクトログラムを表示する準備
    amp=np.abs(spectrogram)    # 振幅を計算
    amp_nonzero=np.where(amp==0,np.nan,amp)   # divide by zero encountered in log10を回避するため0をnanに置き換え
...
def restore_waveform(spectrogram:np.ndarray,n_samples:int,skip_width:int):
    ...
    for i in range(len(spectrogram)):
        ...
        restore_waveform_ndarray[i*skip_width:i*skip_width+n_samples]+=wave_piece
...
def main():
    ...
    # 再計算した波形を出力
    plot_waveform(restore_wavedata_ndarray,sampling_rate,picture_name_option="_restored")
