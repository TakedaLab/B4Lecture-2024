...
def plot_confusion_matrix(true_labels, predicted_labels, file_name, class_names=None):
    """
    プロット混同行列をプロットする関数。
    
    :param true_labels: 正解ラベルのリストまたはNumPy配列
    :param predicted_labels: 予測ラベルのリストまたはNumPy配列
    :param class_names: クラス名のリスト。省略した場合はラベルのユニークな値を使用します。
    """
    # 混同行列の計算
    ...
    # タイトルとラベルの設定
    plt.title(f"Confusion Matrix (Acc. {np.sum(true_labels==predicted_labels)/true_labels.shape[0]*100}%)")
...
if __name__=="__main__":
    ...
    for i in range(model_kinds):
        for j in range(output_kinds):
            forward_result[i,j] = forward(output_data[j],model_data["PI"][i],model_data["A"][i],model_data["B"][i])
    ...
    for i in range(model_kinds):
        for j in range(output_kinds):
            viterbi_result[i,j] = viterbi(output_data[j],model_data["PI"][i],model_data["A"][i],model_data["B"][i])
    # argmaxの結果とプロット
