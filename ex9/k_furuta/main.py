...
class DiffusionModel(pl.LightningModule):
    ...
    def q_sample(
        ...
        a_ = self.alpha_prod[t].view(-1, 1, 1, 1)
        return x0 * a_.sqrt() + noise * (1 - a_).sqrt()
    ...
    def p_sample(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        """
        with torch.no_grad():
            prod_noise = self.forward(x, t)
            a = self.alpha[t].view(-1, 1, 1, 1)
            a_ = self.alpha_prod[t].view(-1, 1, 1, 1)
            x = (x - prod_noise * (1 - a) / (1 - a_).sqrt()) / a.sqrt()
            if t[0].item() != 0:
                x = x + torch.randn_like(x) * (1 - a).sqrt()
            return x
    ...
    def training_step(self, batch, batch_idx):
        ...
        t = torch.randint(
            0, self.num_timesteps, (images.shape[0],), device=self.device
        ).long()
