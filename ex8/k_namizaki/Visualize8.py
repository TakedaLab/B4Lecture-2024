...
class Visualize:
    ...
    def createDirectories(self):
        """Create directories for storing images_z4_42."""
        os.makedirs("./images_z4_42/reconstruction", exist_ok=True)
        os.makedirs("./images_z4_42/latent_space", exist_ok=True)
        os.makedirs("./images_z4_42/lattice_point", exist_ok=True)
        os.makedirs("./images_z4_42/walkthrough", exist_ok=True)

    def reconstruction(self):
        """Visualize the reconstructed images_z4_42."""
        for num_batch, data in enumerate(self.dataloader_test):
            ...
            fig.savefig(f"./images_z4_42/reconstruction/z_{self.z_dim}_{num_batch}.png")
            plt.close(fig)
    ...
    def latent_space(self):
        ...
        for num_batch, data in enumerate(self.dataloader_test):
            ...
            fig_plot.savefig(
                f"./images_z4_42/latent_space/z_{self.z_dim}_{num_batch}_plot.png"
            )
            fig_scatter.savefig(
                f"./images_z4_42/latent_space/z_{self.z_dim}_{num_batch}_scatter.png"
            )
            plt.close(fig_plot)
    ...
    def lattice_point(self):
        ...
        num_image = 25  # How many images_z4_42 per row (column)
        ...
        fig.savefig(f"./images_z4_42/lattice_point/z_{self.z_dim}.png")
        plt.close(fig)
    ...
    def walkthrough(self):
        ...
        for n in range(len(y1_to_y2_list)):
            ...
            for _, im in enumerate(y1_to_y2_list[n]):
                images_z4_42.append([ax.imshow(im, "gray")])
            animation = ArtistAnimation(
                fig, images_z4_42, interval=100, blit=True, repeat_delay=1000
            )
            animation.save(
                f"./images_z4_42/walkthrough/z_{self.z_dim}_{n}.gif", writer="pillow"
            )
            plt.close(fig)
