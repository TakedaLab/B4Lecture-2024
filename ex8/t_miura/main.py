...
class Main:
    ...
    def train_batch(self):
        ...
        self.model.train()
        self.train_KL.append(0)
        self.train_reconstruction.append(0)
        count = 0
        for x, _ in self.dataloader_train:
            lower_bound, _, _ = self.model(x, self.device)
            self.train_KL[-1] += lower_bound[0].cpu().detach().numpy()
            self.train_reconstruction[-1] += lower_bound[1].cpu().detach().numpy()
            ...
            count += 1
        self.train_KL[-1] /= count
        self.train_reconstruction[-1] /= count
        self.num_batch_train -= 1
    ...
    def valid_batch(self):
        ...
        loss = []
        self.model.eval()
        self.val_KL.append(0)
        self.val_reconstruction.append(0)
        count = 0
        for x, _ in self.dataloader_valid:
            lower_bound, _, _ = self.model(x, self.device)
            self.val_KL[-1] += lower_bound[0].cpu().detach().numpy()
            self.val_reconstruction[-1] += lower_bound[1].cpu().detach().numpy()
            loss.append(-sum(lower_bound).cpu().detach().numpy())
            self.num_batch_valid += 1
            count += 1
        self.val_KL[-1] /= count
        self.val_reconstruction[-1] /= count
        self.num_batch_valid -= 1
    ...
    def main(self):
        ...
        ax1.plot(self.train_KL)
        ax1.set_title("train/KL")
        ax2 = fig.add_subplot(2, 2, 2)
        ax2.plot(self.train_reconstruction)
        ax2.set_title("train/reconstruction")
        ax3 = fig.add_subplot(2, 2, 3)
        ax3.plot(self.val_KL)
        ax3.set_title("val/KL")
        ax2 = fig.add_subplot(2, 2, 4)
        ax2.plot(self.val_reconstruction)
        ax2.set_title("val/reconstruction")
