...
class Visualize:
    ...
    def PCA(self):
        ...
        a = a[:, sorted_idx]
        self.aT = a.T
        self.aT_inv = np.linalg.inv(self.aT)
    ...
    def latent_space(self):
        ...
        count = np.zeros(10)
        centroid = np.zeros((10, self.z_dim))
        for num_batch, data in enumerate(self.dataloader_test):
            fig_plot, ax_plot = plt.subplots(figsize=(9, 9))
            fig_scatter, ax_scatter = plt.subplots(figsize=(9, 9))
            _, z_original, _ = self.model(data[0], self.device)
            z_original = z_original.cpu().detach().numpy()
            if self.z_dim > 2:
                z = (self.aT @ z_original.T).T
            else:
                z = z_original
            for k in range(10):
                cluster_indexes = np.where(data[1].cpu().detach().numpy() == k)[0]
                count[k] += np.sum(cluster_indexes)
                centroid[k] += np.sum(z_original[cluster_indexes], axis=0)
                ax_plot.plot(
